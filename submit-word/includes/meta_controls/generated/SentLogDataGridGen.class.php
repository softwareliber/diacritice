<?php
	 ...
	 * listing a collection of SentLog objects.  It includes
	 ...
	 * 
	 */
	class SentLogDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SentLog datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SentLog object, or it can be any QQNode extending from QQN::SentLog().
		 * 
		 ...
		 * sorting using two fields from the SentLog object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SentLog
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intWordSentLogId=<?=urlencode($_ITEM->WordSentLogId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->WordSentLogId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->WordSentLogId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = SentLog::CountAll();
			}
			...
			// Set the DataSource to be a Query result from SentLog, given the clauses above
			$this->DataSource = SentLog::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SentLog-based QQNode.
		 * It will also verify that it is a proper SentLog-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'submit_word_sent_log') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "submit_word_sent_log".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'WordSentLogId': return QQN::SentLog()->WordSentLogId;
				case 'IpAddress': return QQN::SentLog()->IpAddress;
				case 'UserAgent': return QQN::SentLog()->UserAgent;
				case 'DateSent': return QQN::SentLog()->DateSent;
				case 'WordId': return QQN::SentLog()->WordId;
				case 'Word': return QQN::SentLog()->Word;
				default: throw new QCallerException('Simple Property not found in SentLogDataGrid content: ' . $mixContent);
