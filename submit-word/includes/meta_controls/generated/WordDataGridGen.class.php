<?php
	 ...
	 * listing a collection of Word objects.  It includes
	 ...
	 * 
	 */
	class WordDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Word datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Word object, or it can be any QQNode extending from QQN::Word().
		 * 
		 ...
		 * sorting using two fields from the Word object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Word
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intWordId=<?=urlencode($_ITEM->WordId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->WordId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->WordId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Word::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Word, given the clauses above
			$this->DataSource = Word::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Word-based QQNode.
		 * It will also verify that it is a proper Word-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'submit_word_word') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "submit_word_word".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'WordId': return QQN::Word()->WordId;
				case 'Word': return QQN::Word()->Word;
				case 'StatusTypeId': return QQN::Word()->StatusTypeId;
				case 'ProposalCount': return QQN::Word()->ProposalCount;
				case 'LastSent': return QQN::Word()->LastSent;
				default: throw new QCallerException('Simple Property not found in WordDataGrid content: ' . $mixContent);
