<?php
	 ...
	 * listing a collection of WordStatusLog objects.  It includes
	 ...
	 * 
	 */
	class WordStatusLogDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this WordStatusLog datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the WordStatusLog object, or it can be any QQNode extending from QQN::WordStatusLog().
		 * 
		 ...
		 * sorting using two fields from the WordStatusLog object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from WordStatusLog
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intWordStatusLogId=<?=urlencode($_ITEM->WordStatusLogId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->WordStatusLogId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->WordStatusLogId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = WordStatusLog::CountAll();
			}
			...
			// Set the DataSource to be a Query result from WordStatusLog, given the clauses above
			$this->DataSource = WordStatusLog::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a WordStatusLog-based QQNode.
		 * It will also verify that it is a proper WordStatusLog-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'submit_word_word_status_log') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "submit_word_word_status_log".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'WordStatusLogId': return QQN::WordStatusLog()->WordStatusLogId;
				case 'WordId': return QQN::WordStatusLog()->WordId;
				case 'Word': return QQN::WordStatusLog()->Word;
				case 'StatusTypeId': return QQN::WordStatusLog()->StatusTypeId;
				case 'ChangedBy': return QQN::WordStatusLog()->ChangedBy;
				case 'ChangedAt': return QQN::WordStatusLog()->ChangedAt;
				default: throw new QCallerException('Simple Property not found in WordStatusLogDataGrid content: ' . $mixContent);
